...
def embed_pdf_base64(s3_key):
    """Embed PDF using base64 in an iframe."""
    try:
        s3_client = get_s3_client()
        bucket_name = st.secrets["aws"]["bucket_name"]
        full_key = get_full_s3_key(s3_key)
...
def embed_pdf_with_pdfjs(s3_key):
    """Embed PDF using PDF.js (fallback if base64 fails)."""
    try:
        s3_client = get_s3_client()
        bucket_name = st.secrets["aws"]["bucket_name"]
        full_key = get_full_s3_key(s3_key)
        ...
        return f"<p style='color:red'>Error displaying PDF with PDF.js: {str(e)}</p>"

def embed_pdf_with_fallback(s3_key):
    """
    Try to embed PDF using base64 first, fallback to PDF.js if it fails.
    """
    html = embed_pdf_base64(s3_key)
    if html:
        return html
    return embed_pdf_with_pdfjs(s3_key)
